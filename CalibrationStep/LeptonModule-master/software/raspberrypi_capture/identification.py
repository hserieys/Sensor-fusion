...
if __name__ == "__main__":
    ...
    # >>> initialize your Hu-moments accumulator
    hu_records = []
    frame_idx = 0
    ...
    while True:
        ...
        # Denormalize the merged rectangles for all the sensors 
        ...
        # for each fused detection (they all share the same index in each dnrect_* list)
        for idx in range(len(merged_rectangles)):
            # Denormalized rectangles for each sensor
            rects = {
                'r': dnrect_r[idx],
                'g': dnrect_g[idx],
                'b': dnrect_b[idx],
            ...
            # Buffers: (image, do_color2gray?, threshold_flag)
            buffers = {
            ...
            # Process each channel, clamp coords, guard empty, threshold, blit
            for key, ((x1,y1),(x2,y2)) in rects.items():
                ...
                # only slice & threshold if ROI is non‑empty
                ...
                # else: leave full_mask zeros in that region

            # now all mask_full_* are populated safely
            # Resize FLIR mask up to RGB size
            mask_full_f_resized = cv2.resize(
                mask_full_f,
                (rgb_shp[1], rgb_shp[0]),
                interpolation=cv2.INTER_NEAREST
            )
            fused_mask = np.zeros(rgb_shp, dtype=np.uint8)
            for m in (mask_full_r, mask_full_g, mask_full_b, mask_full_d, mask_full_f_resized):
                fused_mask = cv2.bitwise_or(fused_mask, m)
             # Stack all 5 masks (R, G, B, D, F)
            stack = np.stack([
                mask_full_r, mask_full_g,
                mask_full_b, mask_full_d, mask_full_f_resized
            ], axis=-1)  # shape (H, W, 5)

            votes = np.count_nonzero(stack, axis=-1)  # how many sensors flagged each pixel
            ...
            # fused is your final 0/255 mask, dtype=uint8
            contours, hierarchy = cv2.findContours(
                fused_mask,
                cv2.RETR_EXTERNAL,           # retrieves both outer and hole contours
                cv2.CHAIN_APPROX_SIMPLE
            )
           # 2) for each contour, compute geometric & invariant moments
            # 2) for each contour, compute geometric & invariant moments
            hu_list = []
            ...
            for i, cnt in enumerate(contours):
                ...
                # --- fix: use hu_log directly, not obj["hu"]
            ...
            #cv2.putText(fused_mask, label, (int(cx), int(cy)), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,255,0), 1)
            # 8) Display each mask
    ...
    lidar_thread.join()
    flir_thread.join()
    # >>> write out your Hu moments to JSON
    import json
