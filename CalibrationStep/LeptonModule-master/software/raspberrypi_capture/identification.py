...
import joblib                          # Load pre-trained sklearn models (SVM, scaler, label maps)
...
# Load trained SVM classifier on Hu moments, label mapping, and scaler
clf           = joblib.load("svm_hu_classifier.pkl")
label_map     = joblib.load("label_map.pkl")
inv_label_map = {v: k for k, v in label_map.items()}  # Reverse lookup from numeric to string
...
def get_rgb(cap):
    ...
    if flag_b:
        # Otsu threshold + dilation to get edges
        ...
        for i, c in enumerate(contours_blue):
            ...
            if area < 1e2 or area > 5e4:
                continue
            b_contours.append(c)
        # Merge nearby contours and fit ellipses/rectangles
    ...
    # Return timestamp and structured channel data
    channel_data = (
...
def get_lidar(lidar_params):
    ...
    # Convert raw buffer to 16-bit image
    frame_data    = frame.get_buffer_as_uint16()
    depth_img_16  = np.frombuffer(frame_data, dtype=np.uint16).reshape((480, 640))
    # Replace zero-depth with max for display
    depth_img_16[depth_img_16 == 0] = np.max(depth_img_16)
    depth_img_16  = np.fliplr(depth_img_16)  # Flip horizontally

    # Normalize to 8-bit in preallocated buffer
...
def get_flir(flir_params):
    ...
    # Histogram stretch to improve contrast
...
def rectangle2(ellipse, max_x, max_y):
    ...
    mn, mj  = ellipse[1]  # axes lengths
    ang     = ellipse[2] * np.pi/180
    # Compute half-side projections
...
def unite_rectangles(rect):
    ...
    for i in range(len(rect)):
        ...
        for j in range(len(rect)):
            ...
            check = False
            if (x1-bias*dx)<=x01<=(x2+bias*dx) and (y1-bias*dy)<=y01<=(y2+bias*dy):
                check = True
            elif (x1-bias*dx)<=x01<=(x2+bias*dx) and (y1-bias*dy)<=y02<=(y2+bias*dy):
                check = True
            elif (x1-bias*dx)<=x02<=(x2+bias*dx) and (y1-bias*dy)<=y01<=(y2+bias*dy):
                check = True
            elif (x1-bias*dx)<=x02<=(x2+bias*dx) and (y1-bias*dy)<=y02<=(y2+bias*dy):
                check = True
            if check and i!=j:
...
def reduce_near_contours(contours, img):
    """
    Filters out small contours (<5 points), fits ellipses, merges
    those whose centers lie within img.width/10 distance, and returns
    merged contours, ellipses, and bounding rects.
    """
...
def near_point(ellip, ellip_ref):
    """
    For each ellipse in ellip, finds the nearest ellipse center in ellip_ref.
    Returns an array of [x1, x2, y1, y2] pairs.
    """
    near_pt = []
    for i in ellip:
...
def cluster_and_merge_rectangles(rectangles, overlap_threshold=0):
    """
    Groups rectangles into clusters if their shared_area > threshold,
    then merges each cluster into one rectangle.
    """
    n = len(rectangles)
    if n == 0:
        return []
        ...
        if i not in visited:
            ...
            while stack:
                ...
                if curr not in visited:
                    visited.add(curr); cluster.append(rectangles[curr])
...
def to_int_rect(rect):
    """
    Converts float rectangle coordinates to integer tuples.
    """
    (x1,y1),(x2,y2) = rect
...
if __name__ == "__main__":
    ...
    path = os.getcwd()
    folder_list = os.listdir(path)  # List contents (not used further)
    ...
    flir_norm_buffer = np.empty(
        (flir_params["FRAME_HEIGHT"], flir_params["FRAME_WIDTH"]), dtype=np.uint8
    )
    ...
    hu_records = []  # Accumulate Hu moment records
    frame_idx  = 0
    ...
    while True:
        ...
        # Concatenate all normalized rects
        ...
        # For each fused detection region, threshold & build per-channel masks
        for idx in range(len(merged_rectangles)):
            ...
            # Resize FLIR mask to RGB resolution and fuse all 5 masks
            ...
            fused = (votes >= 3).astype(np.uint8) * 255

            # Morphological cleanup of fused mask
            ker = cv2.getStructuringElement(cv2.MORPH_RECT, (5,5))
            fused_mask = cv2.morphologyEx(fused, cv2.MORPH_CLOSE, ker, iterations=1)
            ...
            # Find contours on fused mask and compute Hu moments, classification
            contours, hierarchy = cv2.findContours(fused_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
            hu_list = []
            fused_mask_color = cv2.cvtColor(fused_mask, cv2.COLOR_GRAY2BGR)
            class_counts = {"rocket body": 0, "satellite": 0, "wing plane": 0}
            ...
            for i, cnt in enumerate(contours):
                ...
                features = np.concatenate([hu_log, [aspect_ratio, extent, solidity]])
                features_scaled = scaler.transform([features])
                pred = clf.predict(features_scaled)[0]
                label = inv_label_map[pred]
                class_counts[label] += 1
                object_name = f"{label} #{class_counts[label]}"
                hu_list.append({
                    "id": i,
                    "hu": [float(h) for h in hu_log],
                    "cx": cx,
                    "cy": cy,
                    "label": label
                })
                cv2.putText(fused_mask_color, object_name, (int(cx), int(cy - 10)),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2, cv2.LINE_AA)

            # Display each mask and the fused classified result
            ...
            cv2.imshow("Fused Mask", fused_mask_color)
    ...
    # Cleanup and save Hu moments to JSON
    cv2.destroyAllWindows()
    rgb_thread.join()
    lidar_thread.join()
    flir_thread.join()

    with open("hu_moments.json", "w") as f:
        json.dump(hu_list, f, indent=2)
    print(f"Saved {len(hu_list)} frames of Hu moments to hu_moments.json")
