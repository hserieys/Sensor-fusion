...
if __name__ == "__main__":
    ...
    # 1) Prepare a list to accumulate per-frame stats
    frame_stats = []
    while True:
        with data_lock:
        ...
        for idx in range(len(merged_rectangles)):
            ...
            depth_contribution = count_d/cv2.countNonZero(fused_mask) if cv2.countNonZero(fused_mask)   > 0 else 0.0
            r_contribution = count_r/cv2.countNonZero(fused_mask) if cv2.countNonZero(fused_mask)   > 0 else 0.0
            g_contribution = count_g/cv2.countNonZero(fused_mask) if cv2.countNonZero(fused_mask)   > 0 else 0.0
            b_contribution = count_b/cv2.countNonZero(fused_mask) if cv2.countNonZero(fused_mask)   > 0 else 0.0
            flir_contribution = count_f/cv2.countNonZero(fused_mask) if cv2.countNonZero(fused_mask)   > 0 else 0.0
            # 1) Jaccard for at least one sensor
            jaccard1 = (certainty / float(possibility)) if possibility   > 0 else 0.0
            # 2) Jaccard for at least two sensors
            jaccard2 = (certainty / float(count_at_least_2)) if count_at_least_2 > 0 else 0.0
            # 3) Jaccard for at least three sensors
            jaccard3 = (certainty / float(count_at_least_3)) if count_at_least_3 > 0 else 0.0
            # 4) Jaccard for at least four sensors
            jaccard4 = (certainty / float(count_at_least_4)) if count_at_least_4         > 0 else 0.0
            # 5) Jaccard for at least five sensors
            jaccard5 = (certainty / float(certainty)) if certainty         > 0 else 0.0
        ...
        if cv2.waitKey(1) & 0xFF == ord('q'):
            running = False
            break
    # 3) Once the loop exits, dump everything to JSON
